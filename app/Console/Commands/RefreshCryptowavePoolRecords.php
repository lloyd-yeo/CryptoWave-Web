...
class RefreshCryptowavePoolRecords extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'refresh:cryptopool';
     ...
     * @var string
     */
    protected $description = 'Refresh user`s hashpower record from the old pool.';
    ...
    {
        $all_hashpower_records = UserHashpowerRecord::where('cryptowave_pool_hashrecord', 0)->get();
        foreach ($all_hashpower_records as $all_hashpower_record) {
            $email = $all_hashpower_record->email;
...
"pool_list" :
[
	{"pool_address" : "http://159.89.197.169:3333", 
...
//            $myfile = fopen("/Users/lloyd/config.txt", "w") or die("Unable to open file!");
//            $txt = $config_file_content;
//            fwrite($myfile, $txt);
//            fclose($myfile);

            $file = '/Users/lloyd/config.txt';
            // Open the file to get existing content
            $current = file_get_contents($file);
            // Append a new person to the file
            $current = $config_file_content;
